...
app.use(async (req, res, next) => {
    try {
        ...
        res.status(500).json({ error: 'Erro ao conectar a base de dados.' });
    }
});
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        if (!username || !password) return res.status(400).json({ error: 'Utilizador e password obrigatorios.' });
        if (username.length < 3) return res.status(400).json({ error: 'Utilizador deve ter 3+ caracteres.' });
        if (password.length < 4) return res.status(400).json({ error: 'Password deve ter 4+ caracteres.' });
        ...
        res.json({ success: true, message: 'Conta criada!', username });
    } catch (error) {
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Login efetuado!', username: user.username });
    } catch (error) {
